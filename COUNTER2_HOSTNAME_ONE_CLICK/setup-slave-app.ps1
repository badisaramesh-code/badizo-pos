...
function Test-Server {
  ...
  $serversToTry = @($ServerIp) + @($CandidateServers)
  $serversToTry = $serversToTry |
  ...
  foreach ($server in $serversToTry) {
    $frontendUrl = "http://${server}:3000"
    $healthUrl = "http://${server}:5000/api/health"

    try {
      $frontendResponse = Invoke-WebRequest -UseBasicParsing -Uri $frontendUrl -TimeoutSec 8
      ...
      $failedServers += "${server}: frontend not reachable"
      continue
    }
    ...
    try {
      ...
      $failedServers += "${server}: backend not reachable"
      continue
    }

    $script:ServerIp = $server
    Write-Host "Frontend OK: $frontendUrl" -ForegroundColor Green
    Write-Host "Server OK: $healthUrl" -ForegroundColor Green
    Write-Host "Using server address: $server" -ForegroundColor Green
    return
  }

  throw "Cannot reach Badizo server from Counter2. Tried: $($serversToTry -join ', '). Details: $($failedServers -join '; '). Check same LAN, firewall, or use server IP 192.168.1.9."
}
...
function Write-AppConfig {
  ...
  foreach ($configDir in $configDirs) {
    ...
    if (Test-Path $configPath) {
      $backupPath = Join-Path $configDir "app-config.backup-$(Get-Date -Format 'yyyyMMdd-HHmmss').json"
