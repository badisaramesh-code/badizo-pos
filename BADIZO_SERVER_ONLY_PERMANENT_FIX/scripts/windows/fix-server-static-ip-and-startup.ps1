...
function Wait-ForBadizo {
  foreach ($url in @("http://${TargetIp}:5000/api/health", "http://${TargetIp}:5000", "http://${TargetIp}:3000")) {
    $ok = $false
    for ($i = 1; $i -le 60; $i++) {
...
Add-FirewallRuleIfMissing -DisplayName 'Badizo POS Frontend 3000' -Port 3000

Write-Step 'Installing startup tasks'
& (Join-Path $PSScriptRoot 'install-backend-startup-task.ps1')
& (Join-Path $PSScriptRoot 'install-frontend-startup-task.ps1') -ServerIp $TargetIp
...
Write-Host ''
Write-Host "Server fixed. Counters/admin should use http://${TargetIp}:5000 or http://${TargetIp}:3000" -ForegroundColor Green
