...
set "FRONTEND_LOG=%APP_ROOT%frontend-start.log"
...
start "Badizo Frontend Server" /min /D "%APP_ROOT%" cmd /c ""%NODE_EXE%" scripts\windows\serve-frontend-build.js >> "%FRONTEND_LOG%" 2>&1"

echo.
echo Waiting for Badizo to become ready...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ports=@(5000,3000); foreach($port in $ports){ $ok=$false; for($i=1;$i -le 90;$i++){ $listener=Get-NetTCPConnection -LocalPort $port -State Listen -ErrorAction SilentlyContinue; if($listener){ Write-Host ('READY port ' + $port); $ok=$true; break }; Start-Sleep -Seconds 1 }; if(-not $ok){ Write-Host ('NOT READY port ' + $port); exit 1 } }; for($i=1;$i -le 30;$i++){ try { $r=Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:5000/api/health' -TimeoutSec 3; if($r.StatusCode -ge 200 -and $r.StatusCode -lt 400){ Write-Host 'READY backend health'; exit 0 } } catch { Start-Sleep -Seconds 1 } }; Write-Host 'NOT READY backend health'; exit 1"
if errorlevel 1 (
  echo.
...
echo Frontend fallback: http://%SERVER_IP%:5000
echo Frontend by computer name: http://%COMPUTERNAME%:3000
echo Frontend by computer name fallback: http://%COMPUTERNAME%:5000
