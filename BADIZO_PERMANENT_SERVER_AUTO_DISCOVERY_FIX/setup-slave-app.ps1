...
function Test-Server {
  ...
  if ([string]::IsNullOrWhiteSpace($script:ResolvedServerHost)) {
    throw 'Cannot find Badizo server on this LAN. Start the server computer, run START_BADIZO_SERVER.bat once, and allow Windows Firewall ports 5000 and 3000.'
  }

  $frontend3000 = "http://${script:ResolvedServerHost}:3000"
  $frontend5000 = "http://${script:ResolvedServerHost}:5000"
  if (!(Test-BadizoUrl -Url $frontend3000 -TimeoutSec 8) -and !(Test-BadizoUrl -Url $frontend5000 -TimeoutSec 8)) {
    throw "Badizo backend was found, but frontend did not open at ${frontend3000} or ${frontend5000}."
  }

  Write-Host "Badizo server found: $script:ResolvedServerHost" -ForegroundColor Green
  Write-Host "Frontend OK: $frontend5000 or $frontend3000" -ForegroundColor Green
