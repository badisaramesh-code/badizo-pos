...
function Write-CounterConfig {
  ...
  $appUrl = "http://${ServerHost}:3000"
  if (!(Test-BadizoUrl -Url $appUrl -TimeoutSec 4)) {
...
try {
  ...
  if ([string]::IsNullOrWhiteSpace($serverHost)) {
    throw 'Badizo server was not found on this LAN. On the server computer, run START_BADIZO_SERVER.bat and allow firewall ports 5000 and 3000.'
  }
