...
const TEMPLATE_NAME = 'tsc-244-pro-50x50-two-up.prn';
...
export default function BarcodeStickersView() {
  ...
  async function loadTemplate() {
    try {
      const result = await fetchBarcodeTemplate(TEMPLATE_NAME);
  ...
  async function generatePrn() {
    ...
    try {
      ...
      setStatusMessage(`PRN generated: ${result.output_name}`);
    } catch (err) {
  ...
  return (
    <div className="barcode-grid">
      <section className="panel">
        ...
        <div className="panel-body form-stack">
          ...
          <div className="change-box">
            Sticker Count 2 means one printer row with two 50x50mm stickers. Generate PRN, then send the generated file from barcode/output to the TSC printer.
          </div>
      ...
      <section className="panel">
        <div className="panel-header green"><h2 className="panel-title">50 x 50 mm Two-Up Preview and TSC PRN</h2></div>
        <div className="panel-body barcode-preview-wrap">
          ...
          <pre className="prn-output">{prn || 'Click Generate PRN to create TSC 244 Pro command file from barcode/templates/tsc-244-pro-50x50-two-up.prn'}</pre>
        </div>
      </section>
    </div>
  );
