...
$guide = Join-Path $appRoot 'output\pdf\BADIZO_NEW_STORE_INSTALLATION_GUIDE_TELUGU.pdf'
if (Test-Path -LiteralPath $guide) {
  Copy-Item -LiteralPath $guide -Destination (Join-Path $OutputDir 'BADIZO_NEW_STORE_INSTALLATION_GUIDE_TELUGU.pdf') -Force
}
